...
CLASSES=(
  ...
  "NewQuestionTypeTests"
  "PaginationBoundaryTests"
...
if [[ $# -eq 0 ]]; then
  ...
  echo "  ./run.sh cascading       → CascadingDeleteTests"
  ...
  echo "  ./run.sh protocol        → HttpProtocolTests"
  echo "  ./run.sh integration     → IntegrationTests"
  echo "  ./run.sh invite          → InviteTests"
  echo "  ./run.sh latest          → LatestFeatureTests"
  echo "  ./run.sh ordering        → ListOrderingTests"
  echo "  ./run.sh missing         → MissingCoverageTests"
  echo "  ./run.sh newquestion     → NewQuestionTypeTests"
  echo "  ./run.sh pagination      → PaginationBoundaryTests"
  echo "  ./run.sh password        → PasswordProtectedTests"
  echo "  ./run.sh patch           → PatchTests"
  echo "  ./run.sh performance     → PerformanceTests"
  echo "  ./run.sh profile         → ProfileManagementTests"
  echo "  ./run.sh question        → QuestionManagementTests"
  echo "  ./run.sh ratelimit       → RateLimitingTests"
  echo "  ./run.sh resilience      → ResilienceTests"
  ...
  echo "  ./run.sh token           → TokenRefreshTests"
